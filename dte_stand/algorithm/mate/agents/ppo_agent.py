...
from dte_stand.algorithm.mate.lib.metrics import tf_scprint
...
class PPOAgent(object):
    ...
    def __init__(self,
        ...
        # actor does not support tf.function with some flags (when memory is used)
    ...
    def run_episode(self):
        ...
        for t in range(self.horizon):
            ...
            if t % max(1, self.horizon // 10) == 1 or t == 0 or t == self.horizon - 1:
                print(f"{t} act: {action_numpy} ~ [{action_type}: {act_val}], rwd: {reward}, phi: {self.phi(self.env.G)}")
    ...
    def generate_eval_actor_critic_functions(self):
        ...
        for eval_env_type in self.eval_env_type:
            self.eval_actor[eval_env_type] = Actor(
                self.eval_envs[eval_env_type].G, adj_matrix=self.env.get_adj_matrix(), num_features=self.env.num_features)
    ...
    # @tf.function # 4. Being off is required for out of episode computations (if numpy is used)
    ...
    def act(self, state, select_max=False):
        ...
        greedy_action = tf.argmax(logits, output_type=tf.int32)
        sample_action = dist.sample()

        random_val = tf.random.uniform([], 0, 1)
        action = tf.where(random_val <= self.eps, sample_action, greedy_action)
        return action, dist.log_prob(action), None, msg_iter_done
