...
class Actor(keras.Model):
    def __init__(self,
        ...
        self.cfg = ActorCfg(**cfg) if cfg is not None else ActorCfg()
    ...
    def define_network(self):
        ...
        if self.gat_used:
            ...
            for _ in (range(max(1, self.message_iterations - 1))):
                link_update_layers.append(GATLayer(self.first_hidden_layer_size,
                                            self.num_heads,
                                            self.adj_matrix,
                                            kernel_initializer=self.hidden_layer_initializer,
                                            activation='relu',
                                            dropout_rate=self.dropout_rate))
            link_update_layers.append(GATLayer(self.link_state_size,
                                        1,
                                        self.adj_matrix,
                                        kernel_initializer=self.hidden_layer_initializer,
                                        activation='relu',
            ...
            if self.memory_used:
                self.link_update = link_update_layers
            else:
                self.link_update = keras.models.Sequential(name='link_update')
                for layer in link_update_layers:
                    self.link_update.add(layer)
        else:
            # message
